---
title: "Motorbike Lane Rules in Vietnam"
slug: motorbike-lane-rules-vietnam
description: "Which lanes motorbikes may use in Vietnam under Article 13 of Law 36/2024/QH15: stay in the correct lane, never cross the median, and expect a 600,000-800,000 VND fine for wrong-lane riding."
category: law-licences
tags:
  - "lanes"
  - "rules"
  - "fines"
  - "law"
  - "vietnam"
content_type: legal
search_intent: informational
topic_cluster: law-licences
subcluster: "11.3 traffic-rules"
date_published: 2026-09-22
last_reviewed: 2026-09-22
review_status: VERIFIED
sources:
  - "https://xaydungchinhsach.chinhphu.vn/toan-van-luat-trat-tu-an-toan-giao-thong-duong-bo-119240909105718285.htm — Law on Road Traffic Order and Safety No. 36/2024/QH15, full text (Báo điện tử Chính phủ) — Article 13 (lane use rules)"
  - "https://thuvienphapluat.vn/ma-so-thue/phap-luat-thue/lan-lan-xe-may-phat-bao-nhieu-2025-theo-nghi-dinh-168-nhung-loai-xe-gi-khong-phai-dong-phi-duong-bo-826901-195238.html — Thư viện pháp luật: wrong-lane fines for motorbikes under Decree 168 (điểm d khoản 3 Điều 7, 600,000-800,000 VND)"
  - "https://xaydungchinhsach.chinhphu.vn/toan-van-nghi-dinh-168-2024-nd-cp-quy-dinh-xu-phat-vi-pham-hanh-chinh-ve-trat-tu-atgt-duong-bo-119241231164556785.htm — Decree 168/2024/NĐ-CP, full text (Báo điện tử Chính phủ) — Article 7(3)(d) (wrong-lane fine)"
internal_link_targets: "wrong-way-riding-fine-vietnam, overtaking-rules-motorbike-vietnam, motorbike-speed-limits-vietnam, motorbike-fines-vietnam-overview, right-turn-on-red-vietnam"
---

Motorbikes choose lanes by rule, not by gap. Article 13 of the Law on Road Traffic Order and Safety (No. 36/2024/QH15) requires every vehicle to travel in the correct part of the carriageway and the correct lane for its class — and the fine schedule prices the mistake at 600,000–800,000 VND for wrong-lane riding under Article 7(3)(d) of Decree 168/2024/NĐ-CP, with point deduction attached.

## The basics

On a road with marked lanes, ride inside the lane markings appropriate to your vehicle and direction. On roads without lane markings, keep to the right portion of the carriageway in your direction. Never cross a fixed median between opposing flows — crossing the median is its own violation beyond the wrong-lane rule. Where a road has designated lanes by vehicle type — bus lanes, motorbike lanes on some bridges and newer arterials — the designation governs.

## The Hanoi reality

The gap-riding habit — threading between cars, riding the edge of the bus lane, hopping to the head of the queue between the curb and the first car — is exactly what the lane rules police. Most violations are opportunistic, small, and fined without sympathy, because lane discipline is what keeps the dense flow predictable. Bridges and elevated roads with explicit motorbike lanes are the easy case; the ordinary six-lane arterial at rush hour is where the rule earns its fine.

## Related violations that ride along

Wrong-lane riding pairs naturally with three neighbours in the fine schedule: counterflow riding (riding against the permitted direction — the 4–6 million band), overtaking on the right where not permitted (600,000–800,000 VND), and squeezing across the median. Each is checked independently; a single aggressive manoeuvre can draw more than one clause.

## The practical line

Stay in the marked lane for your direction, treat medians as walls, and if the queue is long, wait — the 600–800 thousand VND shortcut costs more than the minute it saves. For riders from countries where motorbikes filter between lanes freely: Vietnam's flow is denser but the rule is stricter, and checkpoints watch lane behaviour at junction approaches.

*Information, not legal advice. Verified against Law 36/2024/QH15 and Decree 168/2024/NĐ-CP on 2026-09-22.*
