...
- 48 rental articles are published (cluster 1, Batch 2 Slice 1, 2026-09-22 — see docs/matrix/batch-2-status.md), covering all of subclusters 1.1 how-to-rent and 1.2 choosing-bike plus 1.3 through MM-0048. 33 cluster-1 rows remain proposed. All business facts come exclusively from docs/OWNER-FACTS.md.
...
- docs/matrix/batch-1-status.md is the authoritative record for legal Batch 1 progress; docs/matrix/batch-2-status.md is the authoritative record for the rental batch.

Full row-level matrix: docs/matrix/master-matrix.csv. The CSV currently holds 147 committed rows: 81 rental-cluster rows (48 published, 33 proposed) plus the complete 66-row cluster-11 law/licence slice (authored 2026-09-22, 66 published, 0 proposed). Rows for the remaining clusters are authored from this planning framework batch by batch, before each batch is written.
