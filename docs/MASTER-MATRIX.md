...
Full row-level matrix: docs/matrix/master-matrix.csv. The CSV currently holds 567 committed rows (after the batch-11/batch-12 merge 482 -> 532 and the batch-13 merge 532 -> 567, bot commit 9ac0d0f; batch rows preserved verbatim in docs/matrix/batch-11-rows.csv, batch-12-rows.csv and batch-13-rows.csv; merges run via the repo docs-sync workflow - scripts/apply-batch-docs.js): 81 rental-cluster rows (81 published), 45 monthly & long-term rental rows (45 published), the complete 66-row cluster-11 law/licence slice (66 published; renumbered 2026-09-22 from colliding MM-0082–MM-0147 IDs to the unique LAW-0001–LAW-0066 range — see docs/matrix/batch-4-status.md), the complete 55-row cluster-10 safety slice (55 published), 120 cluster-8 maintenance rows (120 published, cluster 8 complete) and 90 cluster-9
 parts/gear rows (90 published; cluster 9 complete) and 110 cluster-3 scooter rows (110 published; cluster 3 complete) and 25 cluster-4 motorcycle rows (25 published; cluster 4 in progress). Rows for the remaining clusters are authored from this planning framework batch by batch, before each batch is written.
