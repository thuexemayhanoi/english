  ...
  'README DEPLOYMENT batch-19');
{
  ...
  let h = -1;
  for (let i = 0; i < lines.length; i++) if (lines[i].trim() === '# CHANGE LOG') { h = i; break; }
  if (h === -1) die('README CHANGE LOG heading not found');
  ...
  if (readme.indexOf(marker) !== -1) {
    ...
    lines.splice(h + 1, 0, '', entry);
    readme = lines.join(NL);
    diag.push('README CHANGE LOG entry added as (' + (max + 1) + ')');
  }
}
