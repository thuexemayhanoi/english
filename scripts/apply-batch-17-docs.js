#!/usr/bin/env node
/**
 * apply-batch-17-docs.js - server-side finalize for Batch 17 (zero dependencies).
 * Applies docs/sync/batch-17-docs-sync.json:
 *   1. master-matrix.csv row merge (ID-row validated; 28 batch-17 rows verbatim, 675 -> 703)
 *   2. README targeted state patches + NEXT RECOMMENDED STEP section replace + changelog entry 39 (idempotent)
 *   3. MASTER-MATRIX.md targeted patches (cluster-5 Part 1 state)
...
function die(msg) {
  ...
  console.error('apply-batch-17-docs: ' + msg);
  process.exit(1);
}
const payload = JSON.parse(fs.readFileSync('docs/sync/batch-17-docs-sync.json', 'utf8'));
...
console.log('apply-batch-17-docs: OK (CSV ' + before + ' -> ' + after + '; README/MM patched)');
