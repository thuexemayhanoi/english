'use strict';
// Batch-25 docs sync: merge docs/matrix/batch-25-rows.csv into docs/matrix/master-matrix.csv,
// apply strictly-validated README.md and docs/MASTER-MATRIX.md state updates from the JSON
// payload, and append the finalization record to docs/matrix/batch-25-status.md.
...
function die(msg) {
  ...
  console.error('apply-batch-25-docs: ' + msg);
  process.exit(1);
}
...
// Tolerant find-string -> regex: whitespace-flexible and dash/em-dash-flexible,
// so display line-wrap artifacts can never break a match.
...
const payload = JSON.parse(fs.readFileSync('docs/sync/batch-25-docs-sync.json', 'utf8'));
...
if (payload.csvMerge) {
  ...
  for (let i = 0; i < ids.length; i++) {
    const want = 'MM-0' + (871 + i);
    if (ids[i] !== want) die('row ' + i + ' has id ' + ids[i] + ' (expected ' + want + ')');
  }
  diag.push('rows file validated: ' + newRows.length + ' rows (MM-0871..MM-0885), 16 fields, Status=published, clean slugs');
...
const statusPath = 'docs/matrix/batch-25-status.md';
...
if (statusDoc.indexOf(marker) === -1) {
  ...
  const section = [
    ...
    marker,
    '',
    '- Batch-25 docs-sync merged the 15 validated batch-25 rows (MM-0871-MM-0885) into docs/matrix/master-matrix.csv (936 -> 951 ID rows) and recorded this finalization. master-matrix.csv is headerless (line 0 is the MM-0001 data row).',
    '- Batch-25 publication record: part-commits aec94fd, 4d95e93 and 2d29b2d; closing inbound-link commit 7553756 (12 existing articles). See DEPLOYMENT STATE in README.md for the verified gate and Pages runs.',
    ''
  ];
  statusDoc += section.join(NL);
  fs.writeFileSync(statusPath, statusDoc);
  diag.push('batch-25-status.md finalization record appended');
} else {
  diag.push('batch-25-status.md finalization record already present');
}

diag.push('OK: ' + mergedNote);
console.log('apply-batch-25-docs: OK — ' + mergedNote + '; README + MASTER-MATRIX updated.');
